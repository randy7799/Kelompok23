...
class Transaksi2 extends ci_controller{
    ...
    function index()
    {
        ...
        {
            $this->model_transaksi2->simpan_barang();
            redirect('transaksi2');
        }
        else
        {
            $data['barang']=  $this->model_buku->list_buku();
    ...
    function laporan()
    {
        if(isset($_POST['submit']))
        {
            ...
            $data['record']=  $this->model_transaksi2->laporan_periode($tanggal1,$tanggal2);
            $this->template->load('template','transaksi/laporan',$data);
        }
        else
        {
            $data['record']=  $this->model_transaksi2->laporan_default();
            $this->template->load('template','transaksi/laporan',$data);
        }
    }
    ...
    function pdf()
    {
        ...
        $pdf->SetFont('Arial','','L');
        $data=  $this->model_transaksi2->laporan_default();
        $no=1;
        $total=0;
